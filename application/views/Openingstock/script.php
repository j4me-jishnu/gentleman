  ...
  $.ajax({
    ...
    success:function(data){
      ...
      $('#OpeningStockHistory').DataTable({
        ...
        "scrollX":true,
        "createdRow": function ( row, data, index ) {
          console.log(data.opening_id);
          $('td', row).eq(3).html('<center><a href="<?php echo base_url();?>Openigstock/single_item_edit/'+data.opening_id+'"><i class="fa fa-edit iconFontSize-medium" ></i></a></center>');
        // "createdRow": function ( row, data, index ) {
        //   $('td', row).eq(3).html('<center><a href="<?php echo base_url(); ?>"Openingstock/test('+data['item_id']+'")><i class="fa fa-edit iconFontSize-medium" ></i></a> &nbsp;&nbsp;&nbsp;<a onclick="return confirmDelete('+data['item_id']+')"><i class="fa fa-trash-o iconFontSize-medium" ></i></a></center>');
        },
        columns: [
          {'data':'item_name'},
          {'data':'item_quantity','render': function(data, type, full, meta){return "<input type='text' value="+data+" readonly> </input>"}},
          {'data':'date'},
          {'data':'date'},
        ],
      });
