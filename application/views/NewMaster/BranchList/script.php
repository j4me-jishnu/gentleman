<script>
  $(function () {
    ...
    $table = $('#branch_table').DataTable( {
        ...
        "serverSide": true,
        "bDestroy" : true,
        "lengthMenu": [100, 200, 300, 400, "ALL"],
        dom: 'lBfrtip',
			buttons: [
				{
					extend: 'copy',
        ...
        "ajax": {
            "url": "<?php echo base_url();?>NewMaster/getBranchList/",
            "type": "POST",
            "data" : function (d) {
        ...
        "createdRow": function ( row, data, index ) {
			...
			$('td', row).eq(5).html('<center><?php $u = $this->session->userdata('user_type'); if($u != 'S'){ ?><a href="<?php echo base_url();?>NewMaster/editBranchList/'+data['branch_id']+'"><i class="fa fa-edit iconFontSize-medium" ></i></a> &nbsp;&nbsp;&nbsp;<a onclick="return confirmDelete('+data['branch_id']+')"><i class="fa fa-trash-o iconFontSize-medium" ></i></a><?php } ?></center>');
...
function confirmDelete(branch_id){
    ...
    if(conf){
        $.ajax({
            url:"<?php echo base_url();?>NewMaster/deleteBranchList",
