...
class Stock extends MY_Controller {
	...
	public function getBstock(){
		...
		// var_dump($data['data'][1]); die(); // Comment this line
		$stock_count_array=array();
		foreach($data['data'] as $item){
			array_push($stock_count_array,$item->total_qty);
		}
		$data["stock_count"]=array_sum($stock_count_array);
		...
		for($i=0;$i<count($data['data']);$i++)
		{
			...
			$data['data'][$i]->total_returns_to_master=$this->Stock_model->getSingleStockReturns($br_id, $data['data'][$i]->item_id_fk);
			$data['data'][$i]->single_to_branch_count=$this->Stock_model->getSingleStockIssuedToBranches($br_id, $data['data'][$i]->item_id_fk);
			$number_of_records=count($data['data'][$i]->single_to_branch_count);
			$records=array();
			if(empty($data['data'][$i]->single_to_branch_count)){
				$data['data'][$i]->single_to_branch_count=0;
			}
			else{
				for($j=0;$j<$number_of_records;$j++){
					array_push($records,$data['data'][$i]->single_to_branch_count[$j]->item_quantity);
				}
				$records = array_map('intval', $records);
				$data['data'][$i]->single_to_branch_count=array_sum($records);
				}
			}
		$json_data = json_encode($data);
		echo $json_data;
	...
	public function updateStocktoStockTable(){
		$id=intval($_POST['id']);
		$quantity=intval($_POST['quantity']);
		$response=$this->Stock_model->add_new_purchase_to_stock($id,$quantity);
		if($response){
			echo "<script>alert('Successfully added to stock')</script>";
			return true;
		}
		else{
			return false;
	...
	// 	$result = $this->Stock_model->get_itemwise_stock_returns(1);
	// }
}
